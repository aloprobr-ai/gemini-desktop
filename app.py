...
HUMAN_PROMPT = """РОЛЬ
Ты — редактор. Твоя работа — переписать текст так, чтобы он читался как
написанный конкретным живым человеком в конкретный день по конкретному
поводу. Не «качественнее», не «профессиональнее» — именно живее.
...
Если образца стиля нет — пиши как толковый человек говорит вслух,
а не пишет в методичку. Жанр и рамки определи по самому тексту:
что в нём обязательно сохранить (термины, даты, цитаты, структуру),
видно из него же.

ГЛАВНОЕ ОГРАНИЧЕНИЕ
Не придумывай фактов, цифр, имён и случаев, которых нет в исходнике.
Живой текст держится на подробностях, но выдуманная подробность — это
враньё, а не живость. Там, где подробность нужна, а взять её неоткуда,
ставь метку [нужна деталь: какая именно] и продолжай.

ВЫРЕЗАТЬ БЕЗ ЖАЛОСТИ
1. Пустые зачины и концовки: «в современном мире», «в наши дни»,
   «в эпоху цифровизации», «итак», «давайте разберёмся», «подводя итог»,
   «в заключение», «надеюсь, это было полезно».
2. Слова-подпорки: «важно отметить», «стоит подчеркнуть», «следует
   учитывать», «таким образом», «безусловно», «на самом деле».
3. Канцелярит: «является», «осуществляет», «производит», «позволяет»,
   «обеспечивает», «способствует», «данный» вместо «этот», отглагольные
   существительные («осуществление проверки» -> «проверил»).
4. Штампы: «играет ключевую роль», «неотъемлемая часть», «широкий спектр»,
   «ряд преимуществ», «эффективное решение», «мощный инструмент»,
   «не только…, но и…», «это не просто X — это Y».
5. Ярлыки машинного письма в английском, если текст английский: delve,
   tapestry, testament, landscape, realm, seamless, robust, leverage,
   foster, underscore, crucial, pivotal, myriad, navigate, unlock,
   elevate, game-changer, «In today's fast-paced world».
6. Тройки. «Быстро, надёжно и удобно» — вычеркни одно слово. Списки
   ровно из трёх пунктов разбей или дополни до четырёх.
7. Двусторонность там, где у человека есть мнение. «С одной стороны…
   с другой стороны…» — выбери сторону и скажи прямо.

СТРУКТУРА
— Списки и подзаголовки убирай, если жанр их не требует. Перечень
  перескажи абзацем: пункты разной длины, одни сливаются, другие
  получают пояснение, третьи вылетают.
— Перечни через точку с запятой («…солдат; …лидер; …») — всегда в прозу.
— Параллельные пункты одной конструкции («Педагог, несущий… /
  Лидер, способный…») не должны идти подряд. Один пересказать глаголом,
  другой свернуть в придаточное, третий выбросить.
— Абзацы не начинай одинаково и не заканчивай выводом-лозунгом.
— Не объявляй, что сейчас будет («Сегодня мы проследим…», «Рассмотрим
  подробнее…»). Просто начинай.

ПАФОС И ОФИЦИОЗ (для исторических, патриотических, учебных тем)
— Вырезать: «несокрушимость», «надёжный гарант», «преемственность
  поколений», «коренной перелом», «сыграл решающую роль», «на протяжении
  веков», «золотыми буквами», «славные традиции». Если мысль за ними
  есть — скажи её простыми словами. Если нет — выброси фразу целиком.
— Цепочки родительных падежей длиннее трёх слов («гарант суверенитета
  и безопасности нашего государства») разбивай или упрощай.
— Больше одного причастного оборота в предложении — переделай одно
  в глагол.
— Абстрактные существительные («деятельность», «подготовка», «система»)
  заменяй тем, что конкретно делали люди: кто, что, когда, где.
— Уважение к теме показывай фактом, а не эпитетом.

ИСТОРИЧЕСКИЙ МАТЕРИАЛ
— Даты, имена, названия бери только из исходника.
— Там, где просится живая деталь (реплика современника, цифра, случай,
  малоизвестный факт), ставь [нужна деталь: …] и уточняй, где её искать:
  мемуары, указ, устав, биография.
— Одно авторское суждение по существу: что ты считаешь главным или
  спорным и почему. Не «важно отметить», а «по-моему».
— Метафора — одна на весь текст, максимум две. Лучше точная и бытовая,
  чем красивая.

РИТМ (самое важное)
— Средняя длина предложения 11–14 слов, но разброс от 2 до 35.
— На каждые 6–8 предложений хотя бы одно короче пяти слов. Вот такое.
— Два предложения подряд одной конструкции — уже перебор. Ломай.
— Абзацы разной длины. Один из пяти — в одну строку.
— Разрешено начинать с «И», «А», «Но», «Зато». Разрешены неполные
  предложения. Разрешено одно отступление в скобках, которое никуда
  не ведёт.
— Не закрывай каждую мысль выводом. Кончилась мысль — следующий абзац.

ЛЕКСИКА
— Выбирай не самое очевидное слово, а самое точное, даже если оно
  простое или грубоватое.
— Если слово точное, повторяй его. Не ищи синоним из вежливости —
  это машинная привычка.
— Смешивай регистры: рядом с термином может стоять разговорное слово.
— Числа не округляй до красивых. «Почти половина» и «47%» звучат
  по-разному, и второе живее.

ДОБАВИТЬ
— Хотя бы одну подробность, которую знает только тот, кто там был:
  время, место, имя, цену, погоду, чью-то реплику. Если её нет
  в исходнике — метка [нужна деталь].
— Одну прямую оценку от первого лица, без смягчений.
— Одно место, где автор признаёт, что ошибся, не знает или сомневается.
— Один переход, сделанный не логической связкой, а просто новой мыслью.

ЧЕГО НЕ ДЕЛАТЬ
— Не вставляй нарочитые опечатки и «ошибки для живости». Это видно.
— Не сыпь эмодзи и восклицательными знаками.
— Не разгоняй объём. Живой текст обычно короче исходного на 10–20%.
— Не применяй одни и те же приёмы в каждом абзаце: приём, повторённый
  по расписанию, сам становится машинным почерком.

ПРОВЕРКА ПЕРЕД ВЫДАЧЕЙ (про себя, в ответ не выводи)
— Прочитай первые слова всех абзацев подряд: нет ли повторов?
— Есть ли хоть одно предложение короче пяти слов? А длиннее 25?
— Остался ли список, тройка, «не только… но и…»?
— Есть ли фраза, которую можно вставить в любой текст на эту тему?
  Если есть — перепиши или убери.
— Нет ли выдуманного факта?

ФОРМАТ ОТВЕТА
1) Готовый текст. Без вступлений и комментариев.
2) Ниже — строка «Нужны детали:» и список меток, если они остались.
3) Ниже — три-четыре слова о том, что ты убрал больше всего.
4) Три самые машинные фразы исходника и во что ты их превратил,
   по одной строке."""

COMPACT_PROMPT = """Сверни весь разговор выше в одну сводку. Она заменит собой историю: дальше ты будешь видеть только её, поэтому всё нужное для продолжения должно попасть внутрь.

Пиши по этим разделам, пропуская те, которым нечего сказать:

1. О чём разговор. Задача и зачем она нужна — два-три предложения.
2. Что решено. Выбранные варианты и причины выбора. Решение без причины через десять реплик снова станет предметом спора.
3. Что сделано. Конкретно: какие файлы созданы и изменены, какие команды сработали, что проверено и чем. Имена, пути, версии, названия функций переноси дословно — по пересказу их не восстановить.
4. Что выяснилось. Ошибки и их причины, тупики, куда ходить не надо. Особенно то, что далось трудом: за это уже заплачено временем.
5. Договорённости и ограничения. Требования, запреты, предпочтения — всё, что сказано один раз и должно действовать дальше.
6. На чём остановились. Что делается прямо сейчас и что следующее.

Правила:
— Точность важнее краткости. Цифры, пути, имена и куски кода — дословно; рассуждения вокруг них сжимай.
— Не пересказывай ход обсуждения («сначала подумали, потом решили»). Оставляй итог.
— Не додумывай. Чего в разговоре не было, того нет.
— Незакрытые вопросы пометь отдельно: они важнее сделанного.
— Никаких вступлений, оценок и похвал. Только сводка.
— Ничего не рисуй и не прикладывай файлов. Нужен текст, и только текст:
  сводка станет историей разговора, и картинке в ней делать нечего."""

# Этим текстом подменяется сама команда после свёртки: модель не должна
# получить приказ «сверни разговор» ещё раз, уже как часть истории.
COMPACT_RESUME = "Ниже — сводка нашего разговора до этого места. Продолжаем с неё."
...
def visible_history(chat):
    ...
    После /compact всё, что было до сводки, остаётся на диске и его
    по-прежнему видно в окне — но модели больше не показывается.
    """
...
def sent_text(msg):
    """Что уходит модели. У команды это развёрнутый запрос, а не «/human»."""
...
class OpenAIBackend:
    ...
    def build_messages(self, chat, system_text):
        ...
        for msg in visible_history(chat):
            if msg["role"] == "user":
                ...
                if sent_text(msg):
                    messages.append({"role": "assistant", "content": sent_text(msg)})
        return messages
    ...
    def build_messages(self, chat, system_text):
        ...
        for msg in visible_history(chat):
            if msg["role"] == "user":
                parts = []
                if sent_text(msg):
                    parts.append({"text": sent_text(msg)})
                ...
                if sent_text(msg):
                    contents.append({"role": "model", "parts": [{"text": sent_text(msg)}]})
...
class Api:
    ...
    def send(self, chat_id, text, images=None):
        ...
        if chat.get("title") in (None, "", "Новый чат"):
            name = ((cmd or {}).get("title") or text
                    or ("Картинка" if len(images) == 1 else "Картинки"))
    ...
    def _expand_command(self, chat, text):
        """Разбирает /команду.

        Возвращает, что показать в окне и что отправить модели, либо None,
        если это обычное сообщение. Незнакомая косая черта командой не
        считается: мало ли, человек начал строку с пути к файлу.
        """
        if not text.startswith("/"):
            return None
        head, _, rest = text.partition(" ")
        name, rest = head.lower(), rest.strip()
        if name not in COMMANDS:
            return None

        if name == "/human":
            target = rest or self._last_model_text(chat)
            if not target:
        ...
        send = COMPACT_PROMPT
        if rest:
            send += "\n\nОсобое внимание удели вот чему: " + rest
        return {"send": send, "kind": "compact"}

    def _apply_compaction(self, chat):
        """Свёртка удалась — прячем от модели всё, что было до сводки.

        На диске и в окне история остаётся целой: свернулось только то,
        что уходит дальше. Поэтому свёртку не жалко — прочитать старое
        по-прежнему можно, достаточно прокрутить вверх.
        """
        msgs = chat.get("messages") or []
        if len(msgs) < 2:
            return
        user, answer = msgs[-2], msgs[-1]
        ...
        if clean != (answer.get("text") or ""):
            answer["text"] = clean
            self.emit({"type": "replace_text", "chatId": chat["id"], "text": clean})
        if not clean:
            return
        user["sendAs"] = COMPACT_RESUME
        user["compacted"] = True
        chat["compactAt"] = len(msgs) - 2
